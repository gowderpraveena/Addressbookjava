...
public class AddressBookDAOImplementation implements AddressBookDAO{
    ...
    @Override
     public void updatePerson(Person person,String name)
     {
         try
       {
           conn = DBConnection.getInstance().getConnect();
           st = conn.createStatement();
            String qry="update AddressBook set name='"+person.getName()+"',mob='"+person.getMob()+"',city='"+person.getEmail()+",email='"+person.getcity()+"' where name='"+name+"'";
           st.executeUpdate(qry);
           conn.close();
        }
    ...
    @Override
    public DefaultListModel getsearch(NameListPanel namePanel, String S) {
       ...
       DefaultListModel<String> dlm = new DefaultListModel<>();
         ...
         conn = DBConnection.getInstance().getConnect();
            Statement stmt;
                 stmt = conn.createStatement();
                 S=S + '%';
            String qry = "select * from AddressBook where name like '"+ S +"' order by name asc";
                 try (ResultSet rs = stmt.executeQuery(qry)) {
                     while (rs.next()){
                         String name = rs.getString(1);
                         dlm.addElement(name);
